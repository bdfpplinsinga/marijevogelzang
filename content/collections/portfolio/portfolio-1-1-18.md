---
id: 33392a51-df89-402a-9131-39096c4c8f04
published: false
blueprint: portfolio
title: Teardrop
form_title: 'Request a quote'
form_description: 'Please let us know the details of your request. We will contact you as soon as possible.'
updated_by: 79296801-252d-4afa-98db-926f50d63dff
updated_at: 1773412801
page_builder:
  -
    id: mljbmkzt
    heading: Teardrop
    heading_type: h2
    text: 'How can small drops of flavour create curious suspense?'
    overlap: true
    type: hero
    enabled: true
    image: portfolio/teardrop/dsc06918.jpg
  -
    id: HadJI5Reqw0dlmtJlP4F5
    text:
      -
        type: paragraph
        content:
          -
            type: text
            marks:
              -
                type: italic
            text: 'Exhibition - Installation'
    type: text
    enabled: true
  -
    id: MqD8gWHNQHBWL_2M_V7pX
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'As a little girl I loved going to the carwash with my father. The automatic brushes and gigantic fluffy rollers were enchanting to me.'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'I enjoyed sitting inside, staying dry and feeling the gentle shakes of the machinery and the wet foamy mess outside. I dreamt to experience a human-wash one day. A similar experience sized fur humans. You stand or sit on a conveyor belt naked or in your bathing suit. Nah… naked. and get scrubbed and bushed by huge wet foamy rolling and moving brushes.'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The carwash experience, in a very light way inspires all my installations with ribbons or string. The haptic quality when engaging with the work is subtle and powerful at the same time. You cannot ignore it but it doesn’t really obstruct you either. It is silently present to stroke you. Or you activate it to stroke you by walking through.'
    type: text
    enabled: true
  -
    id: Y9Yy9d4CAmqYS2LCy1E5f
    images:
      - portfolio/teardrop/dsc07567.jpg
      - portfolio/teardrop/dsc06915.jpg
      - portfolio/teardrop/dsc07565.jpg
    columns: 3
    aspect_ratio: none
    type: image_gallery
    enabled: true
  -
    id: SRgbhnfzQunhB5KK0L-XO
    heading_type: h2
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'We were honoured to have you in Moscow. Within two days of the event we had about 5000-7000 visitors, which exceeded the expected numbers. Teardrop experience was unforgettable!'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Masha Lutsenko'
          -
            type: hardBreak
          -
            type: text
            text: 'Polytechnic museum, Moscow'
    align_content: left
    link:
      link_type: entry
      target_blank: false
      button_type: button
    type: streamer
    enabled: true
    image: portfolio/teardrop/dsc06924.jpg
  -
    id: mJk1rK9gzpRaSZQkzUEG5
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Teardrop is an installation with 5 kilometres of rope. In the center of the rope forest you find a ring with 24 pipettes. All of them are filled with a different water soluble flavour.'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'When you take position in the middle of the installation with your mouth open and your head up you can catch a drop of flavour. The Flavours and the pipets are connected to a few specific strings. You can discover them by looking at strings with a tag on them.'
          -
            type: hardBreak
          -
            type: text
            text: 'The person in the middle cannot reach the strings connected to the pipets so he’s dependant on another person to pull the string and release a drop. Taking away the power over the release of the drop, even when it’s just one single drop, creates a tension in the experience. By shifting the power balance, one drop can seem much bigger than it actually is.'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The strings that are connected to the pipets had tags with poetic lines written on them. They seemed not to make a lot of sense so our brain will try to make a connection with the flavour and the text. Creating a gap between flavour and text made space for visitors to create their own interpretation. It appears the installation is made for two but actually I enjoyed most to be a 3rd actor: to observe the interaction between the two inside.'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The way people communicate when they are collaborating but just a bit far away from each other. To see who is going to be in the middle and who will pull the strings. Those things I find fascinating to observe. It might not be a carwash but now I think of it, perhaps it’s a kind of brainwash with taste.'
    type: text
    enabled: true
  -
    id: XlRKmsbbr7nJixTHkUUXk
    images:
      - portfolio/teardrop/dsc06846.jpg
      - portfolio/teardrop/dsc06704_2.jpg
      - portfolio/teardrop/dsc06859.jpg
      - portfolio/teardrop/teardrop.jpg
      - portfolio/teardrop/dsc06892.jpg
      - portfolio/teardrop/dsc06810_2.jpg
    columns: 3
    aspect_ratio: 4
    type: image_gallery
    enabled: true
  -
    id: FPfZLO2EolrtHv7WA2fA6
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Created for'
          -
            type: hardBreak
          -
            type: text
            text: Dordtyart
          -
            type: hardBreak
          -
            type: text
            text: 'Dordrecht, the Netherlands'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Presented at'
          -
            type: hardBreak
          -
            type: text
            text: 'Eating design solo exhibition'
          -
            type: hardBreak
          -
            type: text
            text: 'OCT gallery, Shenzhen, China'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Presented at'
          -
            type: hardBreak
          -
            type: text
            text: 'Martini time festival'
          -
            type: hardBreak
          -
            type: text
            text: 'Polytechnic museum, Moscow'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Presented at'
          -
            type: hardBreak
          -
            type: text
            text: 'Saudi Design Week'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Riyadh, Saudi Arabia'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Presented at'
          -
            type: hardBreak
          -
            type: text
            text: 'EAT ME exhibition'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'TRAPHOLT museum, Denmark'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'Technical design'
          -
            type: hardBreak
          -
            type: text
            text: 'Robin van der Werff'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'and pictures by Henk van Dijke'
    type: text
    enabled: true
duplicated_from: f7c08ebd-16a7-451f-80d5-476e0dfa4f94
---
