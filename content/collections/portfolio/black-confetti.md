---
id: 947434b5-5eca-4d40-a231-35a5434b6fe7
published: false
blueprint: portfolio
title: 'Black Confetti'
form_title: 'Request a quote'
form_description: 'Please let us know the details of your request. We will contact you as soon as possible.'
updated_by: 79296801-252d-4afa-98db-926f50d63dff
updated_at: 1771408793
page_builder:
  -
    id: mljbmkzt
    heading: 'Black Confetti'
    heading_type: h2
    text: 'How can we get people to experience war food?'
    overlap: true
    type: hero
    enabled: true
  -
    id: _nC-uavcvggpn4pzlwWEM
    text:
      -
        type: paragraph
        content:
          -
            type: text
            marks:
              -
                type: italic
            text: 'Exhibition - Installation - Performance'
    type: text
    enabled: true
  -
    id: vO2dcNMAsW0_YL5psrPxy
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'When Rotterdam was severely bombed at the end of WW2, a so called ‘hunger winter’ emerged and many people died of starvation. The historical museum ‘Het Schielandshuis’ in Rotterdam made an exhibition about the hunger winter and asked me to contribute in an edible way.'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The exhibition was called ‘Black confetti’ referring to an expression that children had proclaimed when the city got bombed. All the black ashes falling from the sky looked like magical black confetti. The hunger winter is infamous among Dutch people because it was a time where people had been eating tulip bulbs. I was curious about what other things were on the menu at the time and went to the resistance museum to get some beautiful handwritten recipes of dishes that could be made with the scarce ingredients of that time.'
    type: text
    enabled: true
  -
    id: YrSdv3Qc_dfRlmkxUnTXf
    heading_type: h2
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The recipes were handwritten but copied from newspapers. At the time, the culinary section of newspapers instructed housewives how to make something from hardly anything. Interestingly I found a recipe for ‘war-cake’ that didn’t replace sugar but did replace wheat flour with potato flour. Of course the hunger winter came gradually and initially only a few items became scarce. People would use ‘ration-coupons’ to get their ration of the food supply.'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'I was curious about the flavours of the time and wanted to bring them to the audience of today. I recreated the recipes literally but I made them into small bites that were presented on a cardboard sheet. Like a ration. When entering the exhibition, visitors would get a ration-coupon and got a cup of surrogate coffee.'
    image: portfolio/black-confetti/black-confetti-3.jpg
    image_position: right
    type: image_text
    enabled: true
  -
    id: LnRyijCp0tvirJ2ZvIxrU
    video: 'https://youtu.be/AKaSNTEGbjE?si=l6KHoZxR18y5sw2x'
    type: video_embed
    enabled: true
  -
    id: xZxXiSyeW_ZN8sGOipM0q
    heading_type: h2
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: '"If my brother would have eaten this, he would still be alive."'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            marks:
              -
                type: bold
              -
                type: italic
            text: 'Visitor of the Black Confetti exhibition'
    image: portfolio/black-confetti/black-confetti-1.jpg
    align_content: left
    link:
      link_type: entry
      target_blank: false
      button_type: button
    type: streamer
    enabled: true
  -
    id: s9cEEfkUrkjCB4joGuizx
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'What I didn’t realise at the time was that some visitors were actually children during the time of war. When they ate the food they tasted flavours they havent been tasting for over 65 years. For some people this opened up locks in their brain and brought back long forgotten memories. Memories of when they were small. When their fathers and mothers were still alive. Memories of being in the kitchen with their siblings during the war. Eating, playing, bombs.'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'The phenomenon of tasting or smelling something and instantly, involuntary retrieving forgotten memories is something most people have experienced before. Scent and flavour have a direct connection to parts of the brain that language is not always able to reach. Marcel Proust wrote about this effect in ‘Remembrance of things past’ when he ate a madeleine soaked in tea and involuntarily remembered a detailed scene with this Aunt Léonie. The Black Confetti project is a good example of the ‘Proust effect’. After this project I have done many tests and events with elderly people and food they ate as a child. The Proust effect is not easy to manipulate as it comes with a fine balance of ingredients and contexts but I have managed to help many people to trigger a memory that was part of their lives but long forgotten. It’s like finding pieces of a puzzle with every bite.'
    type: text
    enabled: false
  -
    id: xlSzqgcK4YunyJrMO6TBl
    heading_type: h2
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'What I didn’t realise at the time was that some visitors were actually children during the time of war. When they ate the food they tasted flavours they havent been tasting for over 65 years. For some people this opened up locks in their brain and brought back long forgotten memories. Memories of when they were small. When their fathers and mothers were still alive. Memories of being in the kitchen with their siblings during the war. Eating, playing, bombs.'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'The phenomenon of tasting or smelling something and instantly, involuntary retrieving forgotten memories is something most people have experienced before. Scent and flavour have a direct connection to parts of the brain that language is not always able to reach. Marcel Proust wrote about this effect in ‘Remembrance of things past’ when he ate a madeleine soaked in tea and involuntarily remembered a detailed scene with this Aunt Léonie. The Black Confetti project is a good example of the ‘Proust effect’. After this project I have done many tests and events with elderly people and food they ate as a child. The Proust effect is not easy to manipulate as it comes with a fine balance of ingredients and contexts but I have managed to help many people to trigger a memory that was part of their lives but long forgotten. It’s like finding pieces of a puzzle with every bite.'
    image_position: left
    type: image_text
    enabled: true
  -
    id: t-116Ydj2kLu4kR5BN5VC
    text:
      -
        type: heading
        attrs:
          level: 2
        content:
          -
            type: text
            text: 'BLACK CONFETTI'
      -
        type: paragraph
        content:
          -
            type: text
            marks:
              -
                type: bold
            text: 'Performed at:'
          -
            type: hardBreak
            marks:
              -
                type: bold
          -
            type: text
            text: 'Museum het Schielandshuis'
          -
            type: hardBreak
          -
            type: text
            text: 'in Rotterdam'
          -
            type: hardBreak
          -
            type: hardBreak
          -
            type: text
            text: 'The film was made for'
          -
            type: hardBreak
          -
            type: text
            text: 'The Future of StorytellingNew York'
          -
            type: hardBreak
          -
            type: text
            text: 'by filmmaker '
          -
            type: text
            marks:
              -
                type: bold
            text: 'Joey de Leeuw'
          -
            type: hardBreak
          -
            type: text
            text: 'and featured his grandmother Jopie and my daughter Juni'
    type: text
    enabled: true
duplicated_from: f7c08ebd-16a7-451f-80d5-476e0dfa4f94
---
